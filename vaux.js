...
client.on("ready", () => {
  ...
  if (time.startsWith("4")) {
    ...
    client.user.setActivity("the voice of the people", { type: "LISTENING" });
...
client.on("message", (receivedMessage) => {
  ...
  // Bot will not repond to its own messages
  if (receivedMessage.author == client.user) {
    return;
  }

  // Generic commands
  if (receivedMessage.guild.id !== "278406764846907392") {
    // Vaux says "Hi"
    if (incomingM.startsWith("vx say hi")) {
    ...
    // Tells presence message
    if (
      incomingM.startsWith("vx what are you doing") ||
      incomingM.startsWith("vx wyd")
    ) {
      const activities = client.user.presence.activities;
      let botActivities = [];
      activities.forEach((activity) => {
        botActivities.push(
          activity.type.toLocaleLowerCase() + " to " + activity.name
        );
      });
  ...
  // Bat Country server only
  if (receivedMessage.guild.id === "735527963306492017") {
    // no server specific commands at current
    return;
  }

  // Catalitos server only
  if (receivedMessage.guild.id === "278406764846907392") {
    ...
    // All commands Vaux's channel only
    if (receivedMessage.channel.id === "755615150299676752") {
